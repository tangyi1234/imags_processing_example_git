...
- (BOOL)canDecodeFromData:(nullable NSData *)data {
    ...
    switch ([NSData sd_imageFormatForImageData:data]) {
        ...
        case SDImageFormatHEIC:
            // Check HEIC decoding compatibility  检查HEIC解码兼容性
            return [[self class] canDecodeFromHEICFormat];
        case SDImageFormatHEIF:
            // Check HEIF decoding compatibility  检查HEIF解码兼容性
...
- (UIImage *)decompressedImageWithImage:(UIImage *)image
    ...
    if (!shouldScaleDown) {//直接解压返回(image是从磁盘中获取到的)
        ...
        if (scaledDownImage && !CGSizeEqualToSize(scaledDownImage.size, image.size)) {
            // if the image is scaled down, need to modify the data pointer as well 如果是按比例缩小的图片,需要修改数据的指针
...
- (nullable UIImage *)sd_decompressedAndScaledDownImageWithImage:(nullable UIImage *)image {
    ...
    if (![[self class] shouldScaleDownImage:image]) {//NO进行解压和解码
        return [self sd_decompressedImageWithImage:image];
    }
...
- (NSData *)encodedDataWithImage:(UIImage *)image format:(SDImageFormat)format {
    ...
    CFStringRef imageUTType = [NSData sd_UTTypeFromSDImageFormat:format];//格式类型
...
+ (BOOL)shouldScaleDownImage:(nonnull UIImage *)image {
    ...
    sourceResolution.width = CGImageGetWidth(sourceImageRef);
    sourceResolution.height = CGImageGetHeight(sourceImageRef);
    float sourceTotalPixels = sourceResolution.width * sourceResolution.height;//图片大小
    float imageScale = kDestTotalPixels / sourceTotalPixels;
    if (imageScale < 1) {//比规定的要小
        shouldScaleDown = YES;
    } else {
        shouldScaleDown = NO;
    }
