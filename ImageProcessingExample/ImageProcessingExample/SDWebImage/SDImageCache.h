...
typedef NS_OPTIONS(NSUInteger, SDImageCacheOptions) {
     ...
     * By default, we do not query disk data when the image is cached in memory. This mask can force to query disk data at the same time.
     默认情况下,我们不能查询磁盘数据图像时缓存在内存中。这个面具可以强制查询磁盘数据在同一时间。
     */
     ...
     * By default, we query the memory cache synchronously, disk cache asynchronously. This mask can force to query disk cache synchronously.
     默认情况下,我们查询内存缓存同步,异步磁盘高速缓存。这个面具可以迫使查询磁盘缓存同步。
     */
    SDImageCacheQueryDiskSync = 1 << 1,
    /**
     * By default, images are decoded respecting their original size. On iOS, this flag will scale down the
     * images to a size compatible with the constrained memory of devices.
     默认情况下,图像解码尊重他们的原始大小。在iOS,这个标志将缩小图像的大小与内存受限的设备兼容。
     */
