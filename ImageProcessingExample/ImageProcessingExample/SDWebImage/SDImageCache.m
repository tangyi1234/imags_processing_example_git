...
- (void)storeImage:(nullable UIImage *)image
         imageData:(nullable NSData *)imageData
            ...
            toDisk:(BOOL)toDisk
        completion:(nullable SDWebImageNoParamsBlock)completionBlock {
    if (!image || !key) {//没有图片或key没有值，都返回一个空。
        if (completionBlock) {
            completionBlock();
        }
        return;
...
// Make sure to call form io queue by caller 确保调用形式io队列由调用者
- (void)_storeImageDataToDisk:(nullable NSData *)imageData forKey:(nullable NSString *)key {
    ...
    // get cache Path for image key 通过key来过去一个路径
...
- (nullable NSData *)diskImageDataBySearchingAllPathsForKey:(nullable NSString *)key {
    ...
    // fallback because of https://github.com/SDWebImage/SDWebImage/pull/976 that added the extension to the disk file name
    // checking the key with and without the extension 检查有无扩展的关键
...
- (nullable NSOperation *)queryCacheOperationForKey:(nullable NSString *)key options:(SDImageCacheOptions)options done:(nullable SDCacheQueryCompletedBlock)doneBlock {
    ...
    if (shouldQueryMemoryOnly) {
        if (doneBlock) {//进行回调图片，且告述是从缓存中提取的。
    ...
    void(^queryDiskBlock)(void) =  ^{
        ...
        @autoreleasepool {
            //通过key获取路径，将路径转化为NSData
            NSData *diskData = [self diskImageDataBySearchingAllPathsForKey:key];
            UIImage *diskImage;
            SDImageCacheType cacheType = SDImageCacheTypeNone;
            if (image) {//缓存中有图片
                // the image is from in-memory cache 内存缓存的图像
                diskImage = image;
                cacheType = SDImageCacheTypeMemory;
            } else if (diskData) {//缓存中没有图片(进入磁盘中查询)
                cacheType = SDImageCacheTypeDisk;
                // decode image data only if in-memory cache missed 解码图像数据只有在内存缓存错过
    ...
    //到硬盘中查询是同步的还是异步的
    if (options & SDImageCacheQueryDiskSync) {//同步
        queryDiskBlock();
    } else {//异步
