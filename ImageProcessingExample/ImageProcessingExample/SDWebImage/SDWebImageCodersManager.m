...
- (UIImage *)decodedImageWithData:(NSData *)data {
    ...
    UNLOCK(self.codersLock);
    for (id<SDWebImageCoder> coder in coders.reverseObjectEnumerator) {
        if ([coder canDecodeFromData:data]) {//检查data数据是否支持UIImage格式
            return [coder decodedImageWithData:data];//就是将data转换为UIImage
        }
    }
    return nil;
}
...
- (NSData *)encodedDataWithImage:(UIImage *)image format:(SDImageFormat)format {
    ...
    UNLOCK(self.codersLock);
    for (id<SDWebImageCoder> coder in coders.reverseObjectEnumerator) {
        if ([coder canEncodeToFormat:format]) {//编码格式是否有
