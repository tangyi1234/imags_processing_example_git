...
- (NSOperation<SDWebImageDownloaderOperationInterface> *)createDownloaderOperationWithUrl:(nullable NSURL *)url
    ...
    // In order to prevent from potential duplicate caching (NSURLCache + SDImageCache) we disable the cache for image requests if told otherwise 为了防止潜在的复制缓存(NSURLCache + SDImageCache)禁用缓存图像请求如果告诉我们
    NSURLRequestCachePolicy cachePolicy = options & SDWebImageDownloaderUseNSURLCache ? NSURLRequestUseProtocolCachePolicy : NSURLRequestReloadIgnoringLocalCacheData; //是否使用缓存(请求缓存)
    ...
    if (self.headersFilter) {//设置Header
    ...
    if (options & SDWebImageDownloaderHighPriority) {//是否为优先下载
        operation.queuePriority = NSOperationQueuePriorityHigh;
    } else if (options & SDWebImageDownloaderLowPriority) {//是否为最后下载
        operation.queuePriority = NSOperationQueuePriorityLow;
    }
    
    if (self.executionOrder == SDWebImageDownloaderLIFOExecutionOrder) {
        // Emulate LIFO execution order by systematically adding new operations as last operation's dependency
        //模仿后进先出添加新操作系统执行顺序,最后操作的依赖
...
- (nullable SDWebImageDownloadToken *)downloadImageWithURL:(nullable NSURL *)url
    ...
    LOCK(self.operationsLock);
    NSOperation<SDWebImageDownloaderOperationInterface> *operation = [self.URLOperations objectForKey:url];
    // There is a case that the operation may be marked as finished, but not been removed from `self.URLOperations`. 有一个操作可能标记为完成,但没有从“self.URLOperations”删除。
    if (!operation || operation.isFinished) {
        //设置请求策略(优先请，是否用请求的缓存等等)
        operation = [self createDownloaderOperationWithUrl:url options:options];
        __weak typeof(self) wself = self;
        operation.completionBlock = ^{//需要删除这个请求的block
        ...
        // Add operation to operation queue only after all configuration done according to Apple's doc. 添加操作操作队列所有配置完成后根据苹果的医生。
        // `addOperation:` does not synchronously execute the `operation.completionBlock` so this will not cause deadlock. ”addOperation:“不同步执行“operation.completionBlock”这不会导致死锁。
    ...
    //初始化Token，加入请求所需的条件(SDWebImageDownloadToken就是一个储存器)
